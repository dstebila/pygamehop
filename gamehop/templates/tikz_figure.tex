...
≤% for proofstep in proof.proof_steps %≥
	...
	≤% if proofstep|type == 'DistinguishingProofStep' %≥
		...
		\node [reduction]
			...
			at (≤≤ loop.index * 4 - 1 ≥≥,1)
			{\texttt{≤≤ proofstep.reduction|classname|texify ≥≥:} \\ \texttt{≤≤ proofstep.experiment.get_name()|texify ≥≥} \\ adversary against \\ \texttt{≤≤ proofstep.schemeName|texify ≥≥}};
		...
		\draw [iscomputationallyindistuishable]
			...
			node[mainarrowlabel,below] {$\mathrm{Adv}^{\texttt{≤≤ proofstep.experiment.get_name()|texify ≥≥}}_{\texttt{≤≤ proofstep.schemeName|texify ≥≥}}(\texttt{≤≤ proofstep.reduction|classname|texify ≥≥})$}
			(hop≤≤ loop.index ≥≥right);
