...
class G2(Crypto.Game):
    ...
    def main(self) -> Crypto.Bit:
        ...
        b = self.Adversary.guess(self.Scheme, pk, ct)
        ret = 0 if b else 1
        return ret
class P2fromP1(P2):
    @staticmethod
...
class G2_expected_result(Crypto.Game):
    ...
    def main(self) -> Crypto.Bit:
        ...
        b = self.Adversary.guess(P2fromP1, pk, ct)
        ret = 0 if b else 1
        return ret

class TestInlineReductionIntoGame(unittest.TestCase):

    def test_G2_for_P2fromP1(self):
        self.assertEqual(
            gamehop.inlining.inline_scheme_into_game(P2fromP1, G2),
            expected_result(G2_expected_result))

    def test_R_into_G1(self):
        print(gamehop.inlining.inline_reduction_into_game(R, G1, G2))
        self.assertEqual(
            gamehop.inlining.inline_reduction_into_game(R, G1, G2),
            expected_result(G2_expected_result))
