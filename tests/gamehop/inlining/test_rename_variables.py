...
class TestRenameVariables(unittest.TestCase):
    def test_basic(self):
        ...
        self.assertEqual(
            ast.unparse(utils.rename_function_body_variables(utils.get_function_def(f_basic), mappings)),
            expected_result(f_basic_expected_result))
    def test_name_collision_in_body(self):
        mappings = {'x': 'a'}
        with self.assertRaises(ValueError):
            utils.rename_function_body_variables(utils.get_function_def(f_basic), mappings)
    ...
    def test_attribute(self):
        mappings = {'a': 'z'}
        self.assertEqual(
            ast.unparse(utils.rename_function_body_variables(utils.get_function_def(f_attribute), mappings)),
