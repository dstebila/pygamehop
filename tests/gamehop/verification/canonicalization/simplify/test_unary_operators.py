...
class TestSimplifyUnaryOperators(unittest.TestCase):
    def test_uadd(self):
        f = gamehop.inlining.internal.get_function_def(f_uadd)
    ...
    def test_nested_usub(self):
        f = gamehop.inlining.internal.get_function_def(f_nested_usub)
        f = simplify.unary_operators(f)
        self.assertEqual(
            ast.unparse(f),
    ...
    def test_not(self):
        f = gamehop.inlining.internal.get_function_def(f_not)
        f = simplify.unary_operators(f)
        self.assertEqual(
            ast.unparse(f),
    ...
    def test_nested_not(self):
        f = gamehop.inlining.internal.get_function_def(f_nested_not)
        f = simplify.unary_operators(f)
        self.assertEqual(
            ast.unparse(f),
